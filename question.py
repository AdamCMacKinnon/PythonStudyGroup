...
python = ['explain the difference betwen a list and a tuple', 'What is a dictionary?', 'When is Python most commonly used?', 'Explain the differnce between a FOR loop and WHILE loop.', 'Explain how classes work in Python, and when you would use them.']
...
react = ['What is REACT?, what are some of its features?','what is JSX?', 'explain State', 'Explain what a component is.', 'what is lifting up state?']

general = ['Explain the difference of global and local scope.', 'What is BIG O?', 'explain the process of pushing files to gitHub.', 'Why is it bad to push to main?', 'What are some examples of bad coding practice?', 'explain recursion', 'order these BIG O time from best to worst: Factorial, Linear, Quadratic, Constant', 'what does NPM stand for?', 'Define Object Oriented Programming']
